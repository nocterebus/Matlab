...
%some images were added to the libraries due to lack of image amount
%number of clusters
K = 77;
...
%% Find descriptors of all classes in training imgs. We will be using 50/50 splits
%find all descriptors in all X classes, cluster to find bag of K words

    
trainRatio = .5;
testRatio = .5;
[trainInd,valInd,testInd] = dividerand(Nimg,trainRatio,0,testRatio);
desc1 = [];
desc2 = [];
desc3 = [];
desc4 = [];
...
for N = trainInd
    ...
    IDX = knnsearch(centers',single(d)');%returns words the descriptors closest represent
...
for N = trainInd
    [f,d] = vl_sift(aic2(:,:,N));
    IDX = knnsearch(centers',single(d)');
    h = histogram(IDX,K);
    v = h.Values;
    v = v';
    H = [H v];
...
%% Confusion Matrix
known = zeros(1,(Nimg/2)*10);
for X = 0:9
    known((X*(Nimg/2))+1:(X+1)*(Nimg/2)) = X+1;
end
...
C = confusionmatStats(known,tested)
